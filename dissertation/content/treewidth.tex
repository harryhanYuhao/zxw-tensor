...
\begin{proof}
  ...
  The algorithm terminates when $T$ becomes an empty graph.
    ...
    \begin{enumerate}
        ...
        % TODO: add references
        \footnote{A leaf is a vertex in a tree with degree 1. The paret of the leaf is the only other vertex connecting to it. All tree must contain at least two leaves.} 
...
\subsection{From Perfect Elimination Order to Tree Decomposition}
...
The constructive proof of Theorem \ref{thm:tree_decomposition_from_perfect_elimination_ordering} is summarised in Algorithm \ref{alg:tree_decomposition_from_peo}.
...
Figure \ref{fig:tree_decomposition_from_perfect_elimination_ordering} is a demonstraction of algorithm \ref{alg:tree_decomposition_from_peo} to construct a tree decomposition from a perfect elimination ordering.
...
\begin{theorem}[Treewidth is the Right Measure for Tensor Contraction]
	Let $G$ be a tensor network with no lose bonds and regarded as a graph and $G^*$ its line graph.
	...
	This complexity can be achieved by finding a tree decomposition of $G^*$.
\end{theorem}
