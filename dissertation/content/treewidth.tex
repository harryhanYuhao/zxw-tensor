...
\begin{figure}[ht]
	...
	\resizebox{0.7\textwidth}{!}{
		\input{./figures/demonstraction_contraction.tikz}
	}
...
Each contraction will takes at most $O(d^{\text{tw}(G^*) + 1})$ time, and the total cost of contracting $G$ is bounded above by $O(n d^{\text{tw}(G^*) + 1})$, where $n$ is the total number of edges.
...
\begin{proof}
	...
	\begin{figure}[ht]
		\centering
		\input{./figures/chord_smaller_cycle.tikz}
...
\begin{proof}[Proof of Lemma \ref{lem:simplicial_vertex}]
	...
	\begin{figure}[ht]
		\centering
		\input{./figures/triangulated_simplicial_existence.tikz}
