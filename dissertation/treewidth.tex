...
We have the following important theorem that connects treewidth and triangulated graphs.

\begin{theorem}[Treewidth and Triangulated Graph]
...
\begin{proof}
    ...
    \begin{enumerate}
    	\item Let $X_l$ be a leaf in $T$. \footnote{A leaf is a vertex in a tree with degree 1.} Let $X_p$ be the parent of $X_l$ in $T$.
		   	We assume that $X_l \setminus X_p$ is a set of one element (justisfied later).
		...
		\item Remove $X_l$ from $T$ and repeat the process until $T$ is empty.
    \end{enumerate}
	...
	An demonstraction of this process is shown in figure 
\end{proof}

\begin{figure}[ht]
	\centering
	\input{./figures/demo_leaf_tree_deomposition.tikz}
	\caption{Demonstration of make a leaf in tree decomposition only contain one more vertex than its parent.}
...
\begin{lemma}[Contracting Makes Neighbours in Line Graph]\label{lem:contracting-makes-neighbours}
	Let $G$ be a graph and $G^*$ its line graph.
	Let $e$ be an edge in $G$ and $v_e$ its corresponding vertex in $G^*$.
	If we contract the edge $e$ in $G$, then all the neighbours of $v_e$ in $G^*$ become neighbours of each other if they are not already.
\end{lemma}
The behaviour of contraction and its corresponding line graph is demonstrated in figure \ref{fig:contracting-makes-neighbours}.
...
\begin{figure}[ht]
	...
	\label{fig:contracting-makes-neighbours}
\end{figure}
